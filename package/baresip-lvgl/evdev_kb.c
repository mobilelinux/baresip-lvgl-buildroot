...
bool evdev_kb_set_file(const char *dev_name)
{
	...
	evdev_kb_fd = open(dev_name, O_RDWR | O_NOCTTY | O_NDELAY);
	if (evdev_kb_fd == -1) {
		perror("unable to open evdev keyboard interface:");
		return false;
	}
	fcntl(evdev_kb_fd, F_SETFL, O_ASYNC | O_NONBLOCK);
	printf("EVDEV_KB: Set file success: %s\n", dev_name);
	return true;
}

void evdev_kb_init(void)
{
	puts("EVDEV_KB: evdev_kb_init called - Auto-detecting KEYBOARD...");
	fflush(stdout);
...
void evdev_kb_read(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
	...
	while (read(evdev_kb_fd, &in, sizeof(struct input_event)) > 0) {
		if (in.type == EV_KEY) {
			...
			// QWERTY Row 1
			case KEY_Q:
				data->key = shift_active ? 'Q' : 'q';
				break;
			case KEY_W:
				data->key = shift_active ? 'W' : 'w';
				break;
			case KEY_E:
				data->key = shift_active ? 'E' : 'e';
				break;
			case KEY_R:
				data->key = shift_active ? 'R' : 'r';
				break;
			case KEY_T:
				data->key = shift_active ? 'T' : 't';
				break;
			case KEY_Y:
				data->key = shift_active ? 'Y' : 'y';
				break;
			case KEY_U:
				data->key = shift_active ? 'U' : 'u';
				break;
			case KEY_I:
				data->key = shift_active ? 'I' : 'i';
				break;
			case KEY_O:
				data->key = shift_active ? 'O' : 'o';
				break;
			case KEY_P:
				data->key = shift_active ? 'P' : 'p';
				break;
			case KEY_LEFTBRACE:
				data->key = shift_active ? '{' : '[';
				break;
			case KEY_RIGHTBRACE:
				data->key = shift_active ? '}' : ']';
				break;
			...
			// QWERTY Row 2
			case KEY_A:
				data->key = shift_active ? 'A' : 'a';
				break;
			case KEY_S:
				data->key = shift_active ? 'S' : 's';
				break;
			case KEY_D:
				data->key = shift_active ? 'D' : 'd';
				break;
			case KEY_F:
				data->key = shift_active ? 'F' : 'f';
				break;
			case KEY_G:
				data->key = shift_active ? 'G' : 'g';
				break;
			case KEY_H:
				data->key = shift_active ? 'H' : 'h';
				break;
			case KEY_J:
				data->key = shift_active ? 'J' : 'j';
				break;
			case KEY_K:
				data->key = shift_active ? 'K' : 'k';
				break;
			case KEY_L:
				data->key = shift_active ? 'L' : 'l';
				break;
			case KEY_SEMICOLON:
				data->key = shift_active ? ':' : ';';
				break;
			...
			// QWERTY Row 3
			case KEY_Z:
				data->key = shift_active ? 'Z' : 'z';
				break;
			case KEY_X:
				data->key = shift_active ? 'X' : 'x';
				break;
			case KEY_C:
				data->key = shift_active ? 'C' : 'c';
				break;
			case KEY_V:
				data->key = shift_active ? 'V' : 'v';
				break;
			case KEY_B:
				data->key = shift_active ? 'B' : 'b';
				break;
			case KEY_N:
				data->key = shift_active ? 'N' : 'n';
				break;
			case KEY_M:
				data->key = shift_active ? 'M' : 'm';
				break;
			case KEY_COMMA:
				data->key = shift_active ? '<' : ',';
				break;
			...
			case KEY_GRAVE:
				...
				break;
			case KEY_1:
				data->key = shift_active ? '!' : '1';
				break;
			case KEY_2:
				data->key = shift_active ? '@' : '2';
				break;
			case KEY_3:
				data->key = shift_active ? '#' : '3';
				break;
			case KEY_4:
				data->key = shift_active ? '$' : '4';
				break;
			case KEY_5:
				data->key = shift_active ? '%' : '5';
				break;
			case KEY_6:
				data->key = shift_active ? '^' : '6';
				break;
			case KEY_7:
				data->key = shift_active ? '&' : '7';
				break;
			case KEY_8:
				data->key = shift_active ? '*' : '8';
				break;
			case KEY_9:
				data->key = shift_active ? '(' : '9';
				break;
			case KEY_0:
				data->key = shift_active ? ')' : '0';
				break;
			case KEY_MINUS:
				data->key = shift_active ? '_' : '-';
				break;
			case KEY_EQUAL:
				data->key = shift_active ? '+' : '=';
				break;
